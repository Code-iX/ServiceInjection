...
internal class SourceCodeFactory
{
    ...
    {
        return GenerateCompilationUnit()
            .NormalizeWhitespace()
            .ToFullString();
    }
    ...
    {
        return TriviaList(Comment("// <auto-generated />"),
    ...
    private static ParameterSyntax CreateParameterFromInjection(Injection injection)
    {
        ...
        if (injection.IsOptional) parameterSyntax = parameterSyntax.WithDefault(EqualsValueClause(LiteralExpression(SyntaxKind.NullLiteralExpression)));
    ...
    private static StatementSyntax CreateAssignmentStatement(Injection injection)
    {
        var assignmentExpression = CreateAssignmentExpression(injection);

        if (!injection.IsOptional) assignmentExpression = AddNullCheck(assignmentExpression, injection.Name);

        return ExpressionStatement(assignmentExpression);
    }

    private static ExpressionSyntax CreateAssignmentExpression(Injection injection)
    {
    ...
    private static ExpressionSyntax AddNullCheck(ExpressionSyntax expression, string parameterName)
    {
        return BinaryExpression(SyntaxKind.CoalesceExpression,
            expression,
            ThrowExpression(ObjectCreationExpression(ParseTypeName(nameof(ArgumentNullException)))
                .WithArgumentList(ArgumentList(SingletonSeparatedList(Argument(InvocationExpression(IdentifierName("nameof")).WithArgumentList(ArgumentList(SingletonSeparatedList(Argument(IdentifierName(parameterName)))))))))));
    }


    private static bool IsGlobalNamespace(ISymbol symbol)
    {
        return symbol is { ContainingNamespace.IsGlobalNamespace: true };
    }
    ...
    private SyntaxList<UsingDirectiveSyntax> CreateUsings()
    {
